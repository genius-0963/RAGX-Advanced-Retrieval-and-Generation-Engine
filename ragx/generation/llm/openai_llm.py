"""
RAGX OpenAI LLM — GPT integration via LangChain.

Provides ``OpenAILLM``, a concrete implementation of ``BaseLLM`` that
wraps ``langchain_openai.ChatOpenAI`` for GPT-family models.
"""
...
class OpenAILLM(BaseLLM):
    """OpenAI GPT integration using ``langchain_openai.ChatOpenAI``.

    Supports both synchronous generation with full token-usage reporting
    ...
    def __init__(
        ...
        Args:
            ...
            api_key:     OpenAI API key.  Falls back to settings / env var.
        ...
        logger.info("openai_llm_ready", model=model)
